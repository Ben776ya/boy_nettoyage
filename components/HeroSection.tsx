...
import { ArrowRight, Phone, Star, Clock, Users, Award } from "lucide-react";
import { PHONE_NUMBER, PHONE_MAIN, COMPANY_TAGLINE, YEAR_FOUNDED, YEARS_EXPERIENCE } from "@/lib/utils";
import ScrollReveal from "./ScrollReveal";

export default function HeroSection() {
  return (
    <section className="relative pt-24 sm:pt-28 md:pt-36 pb-16 md:pb-24 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
          ...
          <ScrollReveal>
            <div>
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-white shadow-lg shadow-primary-navy/5 border border-border/50 mb-5">
              ...
              {/* Tagline */}
              <p className="text-text-gray text-base sm:text-lg leading-relaxed mb-6 max-w-xl">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                <a
                  ...
                  className="group inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 rounded-full bg-white border-2 border-primary-navy/10 text-primary-navy font-semibold hover:border-primary-teal hover:text-primary-teal transition-all duration-300 hover:shadow-lg text-sm sm:text-base"
                >
                  <Phone className="w-4 h-4 sm:w-5 sm:h-5 group-hover:animate-bounce-gentle flex-shrink-0" />
                  <span className="hidden xs:inline">{PHONE_MAIN}</span>
                  <span className="xs:hidden">Appeler maintenant</span>
                </a>
              </div>
            </div>
          </ScrollReveal>

          {/* Right Column - Image (hidden on mobile to keep hero tight, shown sm+) */}
          <ScrollReveal delay={200} className="hidden sm:block">
            <div className="relative overflow-hidden rounded-3xl">
              {/* Decorative ring – desktop only to avoid overflow */}
              <div className="absolute -inset-4 rounded-3xl border-2 border-dashed border-primary-teal/20 animate-spin-slow hidden lg:block"></div>
              ...
              <div className="relative rounded-3xl overflow-hidden shadow-2xl shadow-primary-navy/20">
                ...
                <div className="absolute top-3 right-3 sm:top-4 sm:right-4 glass rounded-2xl px-3 py-2 sm:px-4 sm:py-3 text-center animate-float">
                  <span className="text-2xl sm:text-3xl font-bold gradient-text">{YEARS_EXPERIENCE}</span>
                  <p className="text-[10px] sm:text-xs uppercase tracking-wider text-text-gray font-medium">Ans d&apos;expérience</p>
                </div>

                {/* Floating Badge - Bottom Left */}
                <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 glass rounded-xl sm:rounded-2xl px-3 py-2 sm:px-4 sm:py-3 animate-float-slow">
                  <div className="flex items-center gap-2 sm:gap-3">
                    <div className="flex -space-x-1.5 sm:-space-x-2">
                      ...
                      <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-primary-teal text-white text-[10px] sm:text-xs font-bold flex items-center justify-center border-2 border-white">
                        +
                      </div>
                    </div>
                    <div>
              ...
              {/* Trust badge – lg only */}
              <div className="absolute -bottom-6 -right-6 w-24 h-24 bg-gradient-to-br from-green-cta to-emerald-600 rounded-2xl shadow-xl hidden lg:flex items-center justify-center rotate-12 hover:rotate-0 transition-transform duration-300">
